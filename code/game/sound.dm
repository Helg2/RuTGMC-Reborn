...
/proc/playsound(atom/source, soundin, vol, vary, sound_range, falloff, is_global, frequency, channel = 0, ambient_sound = FALSE)
	...
	for(var/mob/M AS in GLOB.player_list) //maybe do a player_list_by_z for this specifically
		if(ambient_sound && !(M.client?.prefs?.toggles_sound & SOUND_AMBIENCE)) //maybe delete this
			continue
	...
	for(var/mob/M AS in GLOB.aiEyes) // I don't care about copypasta, it's all about optimization
		if(ambient_sound && !(M.client?.prefs?.toggles_sound & SOUND_AMBIENCE)) //maybe delete this
			continue
		var/turf/T = get_turf(M)
		if(!T || T.z != turf_source.z || get_dist(M, turf_source) > sound_range)
